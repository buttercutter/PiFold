...
def patch_neg_dig(x: Union[torch.Tensor], dim: int) -> int:
    return dim if dim >= 0 else x.ndim + dim


def _normalize(tensor: torch.Tensor, dim: int = -1):
    """Better nan handling for cross products."""
...
def _rbf(
    D: torch.Tensor, num_rbf: int, mini: float = 0.0, maxi: float = 20.0
) -> torch.Tensor:
    """D: (...,) -> (..., num_rbf)"""
    # TODO: hypnopump@: rethink RBF expansion vs fourier features cat((sin, cos)·(x * 2^(range(1, N)//2))), x)
    D_mu = torch.linspace(mini, maxi, num_rbf, device=D.device)
    D_mu = D_mu.reshape(*(1,) * D.ndim, -1)
    D_sigma = (mini - maxi) / num_rbf
    RBF = torch.exp(-(((D[..., None] - D_mu) / D_sigma) ** 2))
    return RBF


def _get_rbf(
    A, B, E_idx=None, num_rbf=16, dim: int = -2, eps: float = 1e-6
) -> torch.Tensor:
    """A: (B, L, D), B: (B, L, D), E_idx: ??? -> (B, L, K, num_rbf)"""
    # FIXME: hypnopump@ finish refactor to accept batched process
    dimA = patch_neg_dig(A, dim)
    dimB = patch_neg_dig(B, dim)
    # (B, L, D), (B, L, D) -> (B, L, L)
    D_A_B = (
        (A.unsqueeze(dim=dimA - 1) - B.unsqueeze(dim=dimB))
        .square()
        .sum(dim=-1)
        .add_(eps)
        .sqrt()
    ...
    if E_idx is not None:
        # (B, L, L) -> (B, L, K)
        D_A_B = gather_edges(D_A_B[..., None], E_idx)[..., 0]
    # (B, L, X) -> (B, L, X, num_rbf) where X in {L, K}
    RBF_A_B = _rbf(D_A_B, num_rbf)
    return RBF_A_B


def _orientations_coarse_gl(X: torch.Tensor, E_idx: torch.Tensor, eps: float = 1e-6):
    """X: (B, L, C, D) -> (B, L*C, 3)"""
